#!/usr/bin/env bash
# Build/run the throwaway PBS 4 test server. Published on 127.0.0.1:8007 only.
# Shares port 8007 with the PBS 3 suite (../pbs), so only one may run at a time:
# stop the other container before starting this one.
set -euo pipefail

IMG=tibackup-test-pbs4
NAME=tibackup-test-pbs4
